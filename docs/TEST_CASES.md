...
| :--- | :--- | :--- | :--- | :--- |
| **TC-ADM-01** | **Criar Convite Padrão** | Admin logado | 1. Ir para "Convites".<br>2. Clicar em "Novo Convite".<br>3. Preencher Nome da Família: "Família Silva".<br>4. Selecionar Tipo: "Padrão (Convidado)".<br>5. Adicionar Convidado: "João Silva", Telefone: "11999999999".<br>6. **Inserir Categorias:** "Barato", "Médio".<br>7. Clicar em "Criar". | Convite criado com sucesso. Aparece na lista. Categorias "Barato" e "Médio" são salvas. Slug da URL é gerado. |
| **TC-ADM-02** | **Criar Convite de Padrinho** | Admin logado | 1. Ir para "Convites".<br>2. Clicar em "Novo Convite".<br>3. Selecionar Tipo: "Padrinho".<br>4. Adicionar Convidados.<br>5. Fazer upload de Imagem de Capa personalizada.<br>6. Clicar em "Criar". | Convite criado com o emblema "Padrinhos". Imagem de capa personalizada é exibida na pré-visualização. |
| **TC-ADM-03** | **Atualizar Categorias de Convite** | Convite existente | 1. Editar um convite existente.<br>2. Alterar Categorias de ["Barato"] para ["Caro"].<br>3. Salvar. | Convite atualizado. Convidados que usam este convite devem agora ver APENAS presentes na categoria "Caro". |
...
| ID | Cenário de Teste | Pré-condições | Passos | Resultado Esperado |
| :--- | :--- | :--- | :--- | :--- |
| **TC-ADM-06** | **Criar Presente com Categoria** | Admin logado | 1. Ir para "Presentes".<br>2. Clicar em "Novo Presente".<br>3. Preencher Nome: "Torradeira".<br>4. **Inserir Categoria:** "Barato".<br>5. Definir Visível: Verdadeiro.<br>6. Salvar. | Presente criado. Exibe o emblema da categoria "Barato" no cartão. Status é "Livre" (Disponível). |
...
| **TC-ADM-08** | **Editar Categoria de Presente** | Presente "Barato" existente | 1. Editar presente.<br>2. Alterar Categoria para "Médio".<br>3. Salvar. | Presente atualizado. Convites com Categoria "Barato" NÃO devem mais ver este presente. Convites com "Médio" DEVEM vê-lo. |
...
| ID | Cenário de Teste | Pré-condições | Passos | Resultado Esperado |
| :--- | :--- | :--- | :--- | :--- |
| **TC-PUB-01** | **Carregar Página de RSVP** | Slug Válido | 1. Acessar `/rsvp/{slug}`. | A página carrega com o Nome da Família e Imagem de Capa corretos. A mensagem exibida é a saudação fixa: "{Nome}, temos um convite especial para você". |
...
| **TC-PUB-05** | **Correspondência de Categoria (Positivo)** | Cat. Convite: ["Barato"]<br>Cat. Presente: "Barato" | 1. Confirmar presença.<br>2. Ir para a página de Presentes. | Presente "Barato" **ESTÁ VISÍVEL**. |
| **TC-PUB-06** | **Incompatibilidade de Categoria (Negativo)** | Cat. Convite: ["Barato"]<br>Cat. Presente: "Médio" | 1. Confirmar presença.<br>2. Ir para a página de Presentes. | Presente "Médio" **NÃO ESTÁ VISÍVEL**. |
| **TC-PUB-07** | **Múltiplas Categorias** | Cat. Convite: ["Barato", "Médio"] | 1. Ir para a página de Presentes. | O convidado vê **AMBOS** os presentes das categorias "Barato" e "Médio". |
| **TC-PUB-08** | **Escolher Presente** | Presente Disponível | 1. Clicar em "Escolher esse Presente".<br>2. **Confirmar** no modal de confirmação. | O botão muda para "Escolhido" (Check verde). Presente marcado como Escolhido no backend. Outros convidados o veem como "Escolhido". |
...
- [ ] **Admin:** Criar Convite (Cat "Barato") -> Copiar Link.
- [ ] **Admin:** Criar Presente 1 (Cat "Barato"), Presente 2 (Cat "Médio").
- [ ] **Convidado:** Abrir Link -> Confirmar Presença.
- [ ] **Convidado:** Abrir Presentes -> Verificar que APENAS o Presente 1 está visível.
- [ ] **Convidado:** Escolher Presente 1 -> **Confirmar no modal** -> Verificar sucesso.
